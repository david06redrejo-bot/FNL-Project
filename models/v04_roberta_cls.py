"""v04: Spanish biomedical-clinical RoBERTa with CLS pooling.

Architecture:
1. AutoModel backbone.
2. CLS aggregation: `hidden_states[:, 0, :]`.
3. Dropout(0.1).
...
from src.paths import CHECKPOINTS_DIR, LOGS_DIR, METRICS_DIR, PREDICTIONS_DIR, SUBMISSIONS_DIR, ensure_project_dirs
...
@dataclass
class RobertaCLSConfig:
    """Configuration for the CLS-pooling RoBERTa baseline."""
...
class RobertaCLSClassifier(nn.Module):
    """RoBERTa classifier using the first token (`<s>`) representation."""
...
def make_datasets(config: RobertaCLSConfig):
...
def save_checkpoint(path: Path, model, optimizer, epoch: int, config: RobertaCLSConfig, metrics: dict[str, Any]):
...
def run(config: RobertaCLSConfig) -> dict[str, Any]:
    """Train/evaluate/predict the CLS-pooling baseline."""
    ensure_project_dirs()
    set_seed(config.seed)
    device = get_device()
    if config.debug:
        config = RobertaCLSConfig(
            **{
                **asdict(config),
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Train/evaluate RoBERTa CLS baseline.")
